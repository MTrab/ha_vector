...
class Connection:
    ...
    def connect(self, timeout: float = 10.0) -> None:
        ...
        if hasattr(self._ready_signal, "exception"):
            exc = getattr(self._ready_signal, "exception")
        ...
        try:
            ...
            try:
                ...
                if code is grpc.StatusCode.UNAUTHENTICATED:
                    raise VectorUnauthenticatedException(exc)
                if code is grpc.StatusCode.UNAVAILABLE:
                    raise VectorUnavailableException(exc)
                if code is grpc.StatusCode.UNIMPLEMENTED:
                    raise VectorUnimplementedException(exc)
                if code is grpc.StatusCode.DEADLINE_EXCEEDED:
                    raise VectorTimeoutException(exc)
                raise VectorConnectionException(exc)
